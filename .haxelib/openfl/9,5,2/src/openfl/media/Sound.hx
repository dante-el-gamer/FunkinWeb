...
class Sound extends EventDispatcher
{
	...
	**/
	public function loadPCMFromByteArray(bytes:ByteArray, samples:Int, format:String = "float", stereo:Bool = true, sampleRate:Float = 44100):Void
	{
		...
		audioBuffer.dataFormat = (bitsPerSample == 32 ? F32 : S16);
	...
	**/
	...
	@:noCompletion private function get_length():Int
	{
		...
		if (__buffer != null)
		{
			...
			else if (Reflect.hasField(__buffer, "__srcVorbisFile") && Reflect.field(__buffer, "__srcVorbisFile") != null)
			{
				var srcVorbis:Dynamic = Reflect.field(__buffer, "__srcVorbisFile");
				var samples = Int64.toInt(srcVorbis.pcmTotal());
				return Std.int(samples / __buffer.sampleRate * 1000);
			}
			else
			{
